...
#generate kmers from AA..A to TT..T
...
def MedianString(Dna, k):
    distance = 10e9
    Median = '' 
    for s in generate_kmers(k):
        if distance > DistanceBetweenPatternAndStrings(s, Dna):
            distance = DistanceBetweenPatternAndStrings(s, Dna)
            Median = s
    return Median
